  ...
  return (
    <div style={styles.symptomRow}>
      ...
      <div style={styles.sliderBox}>
        ...
        <span style={{
          ...styles.sliderVal,
          color: value >= 7 ? colors.danger : value >= 4 ? colors.warning : colors.success,
        }}>
          {value}
        </span>
      </div>
  ...
  return (
    <div style={{ ...styles.wrapper, justifyContent: "center" }}>
      <div style={styles.homeInner}>
        ...
        <button onClick={onMyths} style={styles.ghostBtn}>Concussion Myths & Facts →</button>
...
export default function App() {
  ...
  return (
    <>
      ...
      {page === "home"     && <Home onStart={() => setPage("symptoms")} onMyths={() => setPage("myths")} />}
      {page === "symptoms" && <SymptomQuiz onDone={(r) => { merge(r); setPage("reaction"); }} />}
      {page === "reaction" && <ReactionTest onDone={(r) => { merge(r); setPage("eyes"); }} />}
      {page === "eyes"     && <EyeTrackingTest onDone={(r) => { merge(r); setPage("results"); }} />}
      {page === "results"  && <ResultsPage results={results} onRestart={() => { setResults({}); setPage("home"); }} />}
...
const styles = {
  ...
  mythLink: {
    ...
    color: colors.textFaint, fontSize: 14, cursor: "pointer",
  },
  h1: {
  ...
  symptomList: { width: "100%", maxWidth: 640, marginBottom: 40 },
  symptomSectionLabel: {
  ...
  symptomLeft: { display: "flex", alignItems: "center", gap: 12 },
  symptomIcon: { fontSize: 22 },
  symptomLabel: { fontSize: 16, color: colors.textPrimary },
  sliderBox: { display: "flex", alignItems: "center", gap: 12 },
  slider: { width: 130 },
  sliderVal: { fontSize: 17, fontWeight: 800, minWidth: 24, textAlign: "right" },
  primaryBtn: {
    width: "100%",
    maxWidth: 640,
    padding: "18px",
  ...
  homeLogoWrap: { textAlign: "center" },
  homeIcon: { fontSize: 52, display: "block", marginBottom: 12 },
  homeLogoTop: {
  ...
  homeDisclaimer: { fontSize: 12, color: colors.textFaint, textAlign: "center" },
};